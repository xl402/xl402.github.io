...
where parameters `$\alpha_0, \beta_0, m_0, v_0, W_0$` govern the shape of prior distributions. Conditional distributions for `$z_n$` and `$x_n$` are almost identical to one described by the GMM:
